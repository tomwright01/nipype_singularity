...
class UKFTractographyInputSpec(SingularityInputSpec):
    ...
    tracts = File(argstr='--tracts %s',
                  desc="""Output fiber tracts.""",
                  name_source=['dwiFile'],
                  name_template='%s_tracts.vtk',
                  hash_files=False)
    maskFile = File(argstr='--maskFile %s',
                    desc="""Brain mask for diffusion tractography.
                    Tracking will only be performed inside this mask""")
    labels = File(argstr='--labels %s',
                  desc="""A vector of the ROI labels to be used.
                  There are the voxel values where tractography should be
                  seeded. Default: 1""")
    seedsFile = File(argstr='--seedsFile %s',
                     desc="""Seeds for diffusion.
                     If not specified, full brain tractography will be
                      performed, and the algorithm will start from every voxel
                      in the brain mask where the Generalized Anisotropy
                       is bigger than 0.18.""")
...
class UKFTractographyOutputSpec(SingularityOutputSpec):
    tracts = File(desc="Output fiber tracts", exists=True)
